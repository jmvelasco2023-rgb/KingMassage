...
export function BookingsList({ bookings, userId }: any) {
...
function BookingCard({ booking, isExpanded, onToggleExpand, onChatOpen, onCancel, onRate, isPast = false }: any) {
  ...
  // ✅ TRANSPARENCY: Calculate total duration including all admin-added time
  ...
  // ✅ TRANSPARENCY: Track admin-added services
          ...
          {booking.total_price && (
            <div className="bg-blue-50 rounded-xl p-4 space-y-2 border-l-4 border-blue-400">
              ...
              <div className="space-y-1 text-sm">
                ...
                {(hasAdminAddedTime || sessionAddedAddOns.length > 0) && (
                  <p className="text-[10px] text-emerald-600 font-semibold pt-2 flex items-center gap-1">
                    <span>✨</span> Shows additions made by admin
                  </p>
                )}
              </div>
            </div>
